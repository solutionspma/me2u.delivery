...
export async function provisionMerchant(input:{userId:string;email:string;name:string;businessName:string;contact:Record<string,unknown>}){return withTransaction(async client=>{const orgId=randomUUID(),tenantId=randomUUID();await client.query("INSERT INTO app_user(id,external_auth_id,email,display_name) VALUES($1,$2,$3,$4) ON CONFLICT(external_auth_id) DO NOTHING",[input.userId,input.userId,input.email,input.name]);await client.query("INSERT INTO organization(id,name,slug) VALUES($1,$2,$3)",[orgId,input.businessName,`merchant-${input.userId}`]);await client.query("INSERT INTO tenant(id,organization_id,name,slug,environment) VALUES($1,$2,$3,$4,'production')",[tenantId,input.businessName,`merchant-${input.userId}`]);await client.query("INSERT INTO membership(tenant_id,user_id,role) VALUES($1,$2,'MERCHANT_OWNER') ON CONFLICT DO NOTHING",[tenantId,input.userId]);const application=await client.query("INSERT INTO merchant_application(tenant_id,applicant_user_id,business_name,contact,status) VALUES($1,$2,$3,$4,'DRAFT') RETURNING id,status",[tenantId,input.userId,input.businessName,JSON.stringify(input.contact)]);return application.rows[0];});}
