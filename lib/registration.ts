...
export async function provisionMerchant(input:{userId:string;email:string;name:string;businessName:string;contact:Record<string,unknown>}) {
  try {
    ...
    const databaseError = error as { code?: string; constraint?: string; table?: string; column?: string };
    console.error("[merchant-provision-failed]", { code: databaseError.code, constraint: databaseError.constraint, table: databaseError.table, column: databaseError.column });
